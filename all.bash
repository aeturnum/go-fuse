#!/bin/sh
set -eux

for target in "clean" "install" ; do
  for d in fuse fuse/pathfs fuse/test zipfs unionfs \
    example/hello example/loopback example/zipfs \
    example/multizip example/unionfs example/memfs \
    example/autounionfs ; \
  do
    go ${target} github.com/aeturnum/go-fuse/${d}
  done
done

for d in fuse zipfs unionfs
do
  (cd $d && go test github.com/aeturnum/go-fuse/$d && go test -race github.com/aeturnum/go-fuse/$d)
done

make -C benchmark
for d in benchmark
do
  go test github.com/aeturnum/go-fuse/benchmark -test.bench '.*' -test.cpu 1,2
done
